...
include_once(dirname(__FILE__) ."/../../../../plugins/xfusionserver/includes/snmp.php");
 ...
 */
if ($cmd == "index") {
	$total_rows = db_fetch_assoc("SELECT DISTINCT blade FROM `xfusion_server_detail` WHERE host_id=$host_id and blade > -1 ORDER BY blade");
	if (empty($total_rows)) {
 ...
 */
	...
	$total_rows = db_fetch_cell("SELECT COUNT(DISTINCT blade) FROM `xfusion_server_detail` WHERE host_id=$host_id and blade > -1");
	if (empty($total_rows)) {
 ...
 */
	...
	$arr_blades = array();
	$rows = db_fetch_assoc("SELECT DISTINCT blade FROM `xfusion_server_detail` WHERE host_id=$host_id and blade > -1 ORDER BY blade");
	if (empty($rows)) {
	...
	$temperature = _cacti_snmp_get($hostname, $snmp_community, $temperatrue_oid, $snmp_version, $snmp_auth_username, $snmp_auth_password, $snmp_auth_protocol, $snmp_priv_passphrase, $snmp_priv_protocol, $snmp_context, $snmp_port, $snmp_timeout, $ping_retries, SNMP_POLLER);
...
function snmp_get_blade_list($hostname, $snmp_community, $snmp_version, $snmp_auth_username, $snmp_auth_password, $snmp_auth_protocol, $snmp_priv_passphrase, $snmp_priv_protocol, $snmp_context, $snmp_port, $snmp_timeout, $ping_retries) {
	...
	for ($index = 1; $index <= 32; $index++) {
		$blade_presence = _cacti_snmp_get($hostname, $snmp_community, str_replace("#", $index . "", $blade_oid), $snmp_version, $snmp_auth_username, $snmp_auth_password, $snmp_auth_protocol, $snmp_priv_passphrase, $snmp_priv_protocol, $snmp_context, $snmp_port, $snmp_timeout, $ping_retries, SNMP_POLLER);
