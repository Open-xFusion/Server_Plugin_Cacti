...
include_once($config["base_path"] . "/plugins/xfusionserver/lib/ping.php");
...
/**
 * get all servers from xfusion_import_temp,
...
/**
 * get all servers from xfusion_import_temp with status 0,
...
function import_servers($batch_id, $overwrite_exist) {
    ...
    //first save all server to xfusion_server_info table
    foreach ($server_list as &$server) {
...
/**
 * get all XFUSION servers, and update each server info through snmp.
 */
function refresh_server_info() {
    //query status in [success, not start, skipped, in progress]
    $server_list = db_fetch_assoc("SELECT host_id FROM xfusion_server_info WHERE import_status IN (0,1,5,999) AND TIMESTAMPDIFF(HOUR, last_update, NOW()) >= 24");
    ...
    foreach($host_list as $host_info) {
        ...
        if ($server_type != 1 && $server_type != 2) {
            //TODO: test this case
            cacti_log('XFUSION Server plugin: snmp gete server type failed(' . $host_info['host_id'] . '): ' . $host_info["ip_address"]);
            continue;
        }
        ...
        if (($server_type == 1 && $host_info["hw_is_ibmc"] != 1)
            ...
            cacti_log('XFUSION Server plugin: server type changed(' . $host_info['host_id'] . '): ' . $host_info["ip_address"]);
            continue;
        }
        ...
        if ($host_info["hw_is_ibmc"] == 1) {
            ...
            if (!empty($server_detail_snmp)) {
                ...
                if (count($result) > 0) {
                    ...
                    cacti_log('XFUSION Server plugin: server iBMC information updated(' . $host_info['host_id'] . '): ' . $host_info["ip_address"]);
                }
                $import_status = 1;
            } else {
            ...
            if (!empty($server_detail_snmp)) {
                ...
                if (count($result) > 0) {
                    ...
                    cacti_log('XFUSION Server plugin: server HMM information updated(' . $host_info['host_id'] . '): ' . $host_info["ip_address"]);
                }
                $import_status = 1;
            } else {
                $import_status = 4;
