...
function _deal_oid_snmp_get($hostname, $community, $oid, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $port = 161, $timeout = 500, $retries = 0, $environ = SNMP_POLLER) {
    global $config, $host_compony;
    $tmp_oid = $oid;
    if (strstr($oid, '1.3.6.1.4.1.2011.')) {
        // .1.3.6.1.2.1.1.2.0   .1.3.6.1.4.1.58132.2.235 || .1.3.6.1.4.1.2011.2.235
        if (!empty($host_compony[$hostname])) {
            $value = $host_compony[$hostname];
        } else {
            $value = _cacti_snmp_get0($hostname, $community, '.1.3.6.1.2.1.1.2.0', $version, $username, 
                $password, $auth_proto, $priv_pass, $priv_proto, $context, $port, $timeout, $retries, $environ);
            if (strstr($value, '1.3.6.1.4.1.')) {
                $host_compony[$hostname] = $value;
            }
        }
        if (strstr($value, '1.3.6.1.4.1.58132.')) {
            $tmp_oid = str_replace("1.3.6.1.4.1.2011.", "1.3.6.1.4.1.58132.", $oid);
        }
        $host_componys = json_encode($host_compony);
    }
    return $tmp_oid;
}

function _cacti_snmp_get0($hostname, $community, $oid, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $port = 161, $timeout = 500, $retries = 0, $environ = SNMP_POLLER) {
    global $config;
    ...
    if (substr_count($snmp_value, "Timeout:")) {
        cacti_log("WARNING: SNMP Get Timeout for Host:'$hostname', and OID:'$oid', and values:'$snmp_value'", false);
    }
...
function _cacti_snmp_get($hostname, $community, $oid, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $port = 161, $timeout = 1000, $retries = 0, $environ = SNMP_POLLER) {
    global $config;
    // repalce $oid if v6
    $oid = _deal_oid_snmp_get($hostname, $community, $oid, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $port, $timeout, $retries, $environ);
    return _cacti_snmp_get0($hostname, $community, $oid, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $port, $timeout, $retries, $environ);
}

function _cacti_snmp_walk($hostname, $community, $oid, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $port = 161, $timeout = 1000, $retries = 0, $max_oids = 10, $environ = SNMP_POLLER) {
...
function hs_snmp_get(&$server, $oid, $default=null) {
    ...
    if (!array_key_exists($key, $snmp_success)) {
        if ($oid != "1.3.6.1.4.1.2011.2.235.1.1.1.1.0" && $oid != "1.3.6.1.4.1.2011.2.82.1.82.1.1.0"
            && $oid != "1.3.6.1.4.1.58132.2.235.1.1.1.1.0" && $oid != "1.3.6.1.4.1.58132.2.82.1.82.1.1.0") {
...
function hs_snmp_get_table(&$server, $type, &$keys_list, $blade=null, $presence_check=null, $absence_val=0, $need_first_val=null) {
    ...
    foreach ($first_vals as $first_val) {
        ...
        if (strstr($oid, '.58132.')) {
            $first_oid_sub = str_replace(".2011.", ".58132.", $first_oid_sub);
        }
...
/**
 * get server compony by send a snmp query, use oid .1.3.6.1.2.1.1.2.0
 */
function hs_snmp_get_server_compony(&$server) {
    global $hs_oids;
    $server["snmp_version"] == 1;
    $val = hs_snmp_get($server, '.1.3.6.1.2.1.1.2.0', null);
    if ($val == null || $val == "") {
        return -1;
    }
    // xfusion
    if (strstr($val, '1.3.6.1.4.1.58132.')) {
        return 2;
    }
    // hw
    return 1;
...
function hs_snmp_get_ibmc_info(&$server, $category_nums=null){
    ...
    if ($category_nums == null || in_array(1, $category_nums)) {
        ...
        $first_oid = $hs_oids["ibmc"]["firmware_type"];
        $firmware_types = hs_snmp_walk($server, $first_oid);
        $first_oid_sub = substr($first_oid, strpos($first_oid, ".2011."));
        ...
        foreach ($firmware_types as $firmware_type) {
            ...
            if ($ftype == 4 && !preg_match("/^BIOS$/i", $firm_name, $matches))
                continue;
            if ($ftype == 1 && preg_match("/^Available/i", $firm_name, $matches))
                continue;
