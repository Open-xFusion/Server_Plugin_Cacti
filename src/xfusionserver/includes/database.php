 ...
 */
function xfusionserver_upgrade_database () {
    global $config, $database_default;
    include_once($config['library_path'] . '/database.php');

    xfusionserver_setup_database ();

    include_once($config['base_path'] . '/plugins/xfusionserver/setup.php');
    include_once('./functions.php');
    $v = plugin_xfusionserver_version();

    $oldv = read_config_option('plugin_xfusionserver_version');

    if ($oldv < .1) {
        db_execute('INSERT INTO settings (name, value) VALUES ("plugin_xfusionserver_version", "' . $v['version'] . '")');
        $oldv = $v['version'];
    }
}
/**
 ...
 */
function xfusionserver_setup_database () {
    ...
    $data['type'] = 'MyISAM';
    $data['comment'] = 'Table of XFUSION Server iBMC infomation';

    //_plugin_db_table_create only create the table when it's not exist
    _plugin_db_table_create ('xfusionserver', 'xfusion_server_info', $data);
    ...
    $data['type'] = 'MyISAM';
    $data['comment'] = 'Table of XFUSION Server detailes information';
    _plugin_db_table_create ('xfusionserver', 'xfusion_server_detail', $data);
    ...
    _plugin_db_table_create ('xfusionserver', 'xfusion_import_temp', $data);

    // add two columns of is_xfusionserver, import_status, import_result
    _plugin_db_add_column ('xfusionserver', 'host', array('name' => 'hw_is_ibmc', 'type' => 'tinyint(2)', 'NULL' => false, 'default' => '0', 'after' => 'disabled'));
    _plugin_db_add_column ('xfusionserver', 'host', array('name' => 'hw_is_hmm', 'type' => 'tinyint(2)', 'NULL' => false, 'default' => '0', 'after' => 'hw_is_ibmc'));
    _plugin_db_add_column ('xfusionserver', 'host', array('name' => 'hw_is_xfusion_server', 'type' => 'varchar(4)', 'NULL' => false, 'default' => '0', 'after' => 'hw_is_hmm'));
    _plugin_db_add_column ('xfusionserver', 'host', array('name' => 'hw_last_update', 'type' => 'datetime', 'NULL' => true, 'after' => 'hw_is_xfusion_server'));
...
function _plugin_db_table_create ($plugin, $table, $data) {
    ...
    if (!in_array($table, $tables)) {
        ...
        if (db_execute($sql)) {
            cacti_log("XFUSION Server plugin:table created: $table", false, "xfusionserver");
        }
    }
}