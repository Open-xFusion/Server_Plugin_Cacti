...
function hs_render_header() {
    ...
    if (strpos($config['cacti_version'], "0.") === 0) {
        include_once("./plugins/xfusionserver/general_header.php");
        return;
    }
...
/**
 * save server list to xfusion_server_list table,
...
function hs_save_import_server_list($batch_id, &$server_ip_list) {
    ...
    $exist = db_fetch_row("SELECT batch_id FROM xfusion_import_temp WHERE batch_id='$batch_id'");
    if ($exist) return;

    $success = 0;
    foreach ($server_ip_list as $ip_address) {
        $result = db_execute("INSERT INTO xfusion_import_temp (batch_id, ip_address, status, import_status, last_update) 
...
function hs_set_server_ping_status($batch_id, $server_ip, $ping_status, $latency) {
    $sql = "UPDATE xfusion_import_temp
...
function hs_set_server_import_status($batch_id, &$server, $import_status, $pid=null) {
    ...
    if (isset($server) && array_key_exists("host_id", $server)) {
        $sql = "UPDATE xfusion_server_info
...
function hs_get_import_status($batch_id, $server_ip=null) {
    $sql = "SELECT ip_address, status, import_status FROM xfusion_import_temp 
            WHERE batch_id='" .$batch_id. "'";
...
function hs_remove_import_server($batch_id) {
    db_execute("DELETE FROM xfusion_import_temp WHERE batch_id='" .$batch_id. "'");
}
 ...
 */
function hs_get_host_template($host_type, $host_info) {
    if ($host_type == 1) {
        $template_hash = 'f7675b948e98b3840683f63750d27aa4';
        if ($host_info && !empty($host_info["ip_address"])) {
            $val = hs_snmp_get_server_compony($host_info);
            if ($val == 2) {
                $template_hash = '3827831530331d69e49894b0e7f7c008';
            }
        }
    } elseif ($host_type == 2) {
    ...
    foreach($detail_info as $instance=>$values) {
        if (is_array($values)) {
            ...
            foreach($values as $key=>$value) {
                $sql = "
                    INSERT INTO xfusion_server_detail (host_id,blade,instance,`key`,category_num,`value`,oid,last_update)
                    VALUES ("
                    .$host_id. ","
                    .$blade_index. ",'"
                    .$instance. "','"
...
function hs_update_server_info($host_id, &$server_info) {
    ...
    $find = false;
    $sql = "UPDATE xfusion_server_info SET ";
...
function hs_init_server_info(&$server, $host_id, $import_status) {
    ...
    if (!empty($exist_server)) {
        $sql = "UPDATE xfusion_server_info SET import_status=$import_status, last_update=NOW() WHERE host_id=$host_id";
    } else {
        $sql = "INSERT INTO xfusion_server_info (host_id, import_status, sys_guid, model, hostname, 
        ...
        cacti_log("XFUSION Server plugin: server created: " .$server["ip_address"], false, "xfusionserver");
    }
...
/**
 * save XFUSION server ibmc info to database
 */
function hs_save_ibmc_info($host_id, &$ibmc_info, $category_nums=null) {
    if ($category_nums == null || in_array(1, $category_nums)) {
        if (empty($ibmc_info["ip_address"]) || empty($host_id)) {
            cacti_log("XFUSION Server plugin: BAD ip address or host_id!", false, "xfusionserver");
            return;
        }
        ...
        $sql = "SELECT cpu_count,mem_size,disk_size FROM xfusion_server_info
        ...
        $sql = "DELETE FROM xfusion_server_info WHERE host_id=" . $host_id;
        db_execute($sql);

        $general_info = $ibmc_info["general"];
        $sql = "INSERT INTO xfusion_server_info (host_id, status, import_status, sys_guid, model, hostname, ip_address, ibmc_ver, cpu_count, mem_size, disk_size,last_update)
            VALUES ("
            . $host_id . ","
            . $ibmc_info["status"] . ",1,'"
            . $ibmc_info["sys_guid"] . "','"
        ...
        debug_log_insert("XFUSION Server plugin", "Rack server info updated: " . $host_id);
    }

    $sql = "DELETE FROM xfusion_server_detail WHERE host_id=" .$host_id;
    ...
    if ($category_nums == null || in_array(3, $category_nums)) {
        ...
        db_execute("UPDATE xfusion_server_info SET mem_size=" .$ibmc_info['memory']['mem_size']. " WHERE host_id=" .$host_id);
    }
    if ($category_nums == null || in_array(4, $category_nums)) {
        hs_save_server_details($host_id, 1, -1, 4, $ibmc_info['cpu']);
        db_execute("UPDATE xfusion_server_info SET cpu_count=" .$ibmc_info["cpu"]['cpu_count']. " WHERE host_id=" .$host_id);
    }
    if ($category_nums == null || in_array(5, $category_nums)) {
        hs_save_server_details($host_id, 1, -1, 5, $ibmc_info['disk']);
        db_execute("UPDATE xfusion_server_info SET disk_size=" .$ibmc_info['disk']['disk_size']. " WHERE host_id=" .$host_id);
    }
...
/**
 * save XFUSION server hmm info to database
 */
function hs_save_hmm_info($host_id, &$hmm_info, $blades=null, $category_nums=null) {
    if ($category_nums == null || in_array(1, $category_nums)) {
        ...
        $sql = "INSERT INTO xfusion_server_info (host_id, status, import_status, sys_guid, model, hostname, ip_address, smm_ver, cpu_count, mem_size, disk_size, blade_count, last_update)
            VALUES ("
            . $host_id . ","
            . $hmm_info["status"] . ",1,'','"
            . $hmm_info["model"] . "','"
        ...
        debug_log_insert("XFUSION Server plugin", "Blade server info updated: " . $host_id);
    }
...
function hs_get_server_list($filter, $status, $server_type, $sortc, $sortd, $page, $rows) {
    ...
    $total_rows = db_fetch_cell("SELECT count(*) FROM xfusion_server_info AS t INNER JOIN host AS h ON (t.host_id=h.id) $sql_where");
    $server_list = db_fetch_assoc("SELECT t.*, h.status AS host_status, h.disabled, h.hw_is_ibmc, h.hw_is_hmm, h.hw_last_update 
            FROM xfusion_server_info AS t INNER JOIN host AS h ON (t.host_id=h.id)
            $sql_where
...
function hs_get_server_byid($host_id) {
    $result = db_fetch_row("
            ...
            FROM xfusion_server_info AS t LEFT JOIN host AS h ON (t.host_id=h.id)
...
/**
 * read server detailed information from xfusion_server_detail table,
...
function hs_get_server_details($server_type, &$result, $host_id, $blades, $category_nums, $refresh_status=false, &$snmp_info) {
    ...
    $sql = "
        SELECT * FROM xfusion_server_detail
        WHERE host_id=$host_id";
...
/**
 * get XFUSION server iBMC information
 */
...
/**
 * get XFUSION server HMM information
 */
...
function hs_remove_server($host_id) {
    ...
    debug_log_insert("XFUSION Server plugin", "server removed: " . $host_id);
}
