...
include_once("./plugins/xfusionserver/includes/batch_form.php");
...
switch ($action) {
    ...
    case "import":
        ...
        exec_background($command_string, $debug_str. "\"" . $config["base_path"] . "/plugins/xfusionserver/includes/jobs.php\" --action=import_server --overwrite=" .$overwrite_exist. " --batch_id=" .$batch_id);
...
function save_host_list($batch_id, &$server_list, &$exist_hosts) {
    ...
    foreach($server_list as $server) {
        ...
        if (array_key_exists($server["ip_address"], $exist_hosts)) {
            ...
            $sql = "UPDATE host SET "
                . "host_template_id=0,"
                . "description='XFUSION Server - " . $server['ip_address'] . "',"
...
function batch_upload_form()
{
    ...
    <tr class="even noprint">
        ...
        <td nowrap>
            ...
            <a href="<?php echo $config['url_path']; ?>plugins/xfusionserver/static/batch-import-template-v3.csv">
    ...
    <script type="text/javascript" src="<?php echo $config['url_path']; ?>plugins/xfusionserver/static/custom-alert.js"></script>
    <script type="text/javascript" src="<?php echo $config['url_path']; ?>plugins/xfusionserver/static/jquery.ajaxfileupload.js"></script>
