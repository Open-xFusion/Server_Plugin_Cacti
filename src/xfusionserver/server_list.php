...
include_once("./plugins/xfusionserver/includes/functions.php");
include_once("./plugins/xfusionserver/lib/safe_input.php");
...
function server_list_form()
{
    ...
    if (isset($_REQUEST["clear_x"])) {
        kill_session_var("sess_xfusionservers_filter");
        kill_session_var("sess_xfusionservers_rows");
        kill_session_var("sess_xfusionservers_sort_column");
        kill_session_var("sess_xfusionservers_sort_direction");
        kill_session_var("sess_xfusionservers_current_page");
        kill_session_var("sess_xfusionservers_status");
        kill_session_var("sess_xfusionservers_server_type");
    ...
    if ((!empty($_SESSION["sess_xfusionservers_status"])) && (!empty($filter_status))) {
        if ($_SESSION["sess_xfusionservers_status"] != $filter_status)
            $_REQUEST["page"] = 1;
    }

    if ((!empty($_SESSION["sess_xfusionservers_server_type"])) && (!empty($filter_type))) {
        if ($_SESSION["sess_xfusionservers_server_type"] != $filter_type)
            $_REQUEST["page"] = 1;
    }

    /* remember these search fields in session vars so we don't have to keep passing them around */
    load_current_session_value("filter", "sess_xfusionservers_filter", "");
    load_current_session_value("rows", "sess_xfusionservers_rows", "-1");
    load_current_session_value("sort_column", "sess_xfusionservers_sort_column", "hostname");
    load_current_session_value("sort_direction", "sess_xfusionservers_sort_direction", "ASC");
    load_current_session_value("page", "sess_xfusionservers_current_page", "1");
    load_current_session_value("status", "sess_xfusionservers_status", "-1");
    load_current_session_value("server_type", "sess_xfusionservers_server_type", "1");
    ...
    <table width="100%" style="margin-top:5px;margin-bottom:5px;">
        <tr>
            <td rowspan="2"><a href="https://www.xfusion.com">
                <img src="<?php echo $config['url_path']; ?>plugins/xfusionserver/static/images/logo.png" alt="XFUSION"></a></td>
            <td align="right">
                <a href="<?php echo $config['url_path']; ?>plugins/xfusionserver/batch_import.php" class="linkEditMain">Batch Import Servers</a>
            </td>
        </tr>
        <tr>
            <td align="right">
                <a href="<?php echo $config['url_path']; ?>plugins/xfusionserver/scan_server.php" class="linkEditMain">Auto Scan Servers</a>
            </td>
        </tr>
    </table>
    ...
    <?php
    html_start_box("<strong>XFUSION Servers List</strong>", "100%", "", "3", "center", "");
    ?>
        <tr>
            <!--
